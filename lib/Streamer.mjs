...
export default class Streamer {
  ...
  async streamCommand(requestedRate, requestedRange) {
    // I - streaming
    const command = new Command({command: 'I ' + requestedRate + ' ' + requestedRange, timeout: 3000, responsePrefix: ['OP:']});
    await command.exec(this.device);
    for (let response of command.responses) {
  ...
  receiveLine(line) {
    ...
    // Find out if the line is entirely hex characters
    let allHex = true;
    ...
    if (allHex && line.length >= 16 && (line.length - 16) % 12 == 0) { // 8+4+4+25*12+2=318, -2 CRLF = 316
  ...
  packetReceived(packet) {
    ...
    if (false) {
      ...
      if (this.debugLastPacketTime) {
        ...
        console.log("SAMPLES: After " + elapsed + "s, received " + packet.samples.length + " samples (" + rate + " Hz) -- packet delta time is " + packetElapsed + "s")
      }
