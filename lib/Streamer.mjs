...
packet.timestamp = packet.timestamp & 0x00ffffff;
      ...
      log('STREAMER: RECV-HR: ' + (line.length - 1) + '/' + dataLength + ' bytes, ' + (dataLength / 8) + ' values (allHex=' + allHex + '): ' + line.substr(1));

      /*
      if (bytes.length <= 0) {
        log('STREAMER: RECV-HR (empty)');
      } else {
        log('STREAMER: RECV-HR (' + bytes.length + ' bytes = ' + (bytes.length / 4) + ' values)');
      }
      */
