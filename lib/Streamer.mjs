...
export default class Streamer {
  constructor(device) {
    ...
    this.lastBpm = null;
    this.hrValuesBuffer = [];
    this.hrLastReceived = null;
    this.hrSampleIndex = 0;
  }
  ...
  async manualCommand(commandString) {
    const command = new Command({command: commandString, timeout: 3000, responsePrefix: ['']});
    await command.exec(this.device);
    return true;
  }
...
packet.timestamp = packet.timestamp & 0x00ffffff;
      ...
      // Add any HR values received
      if (this.hrValuesBuffer.length > 0) {
        packet.hrValues = this.hrValuesBuffer.splice(0);
        packet.bpm = packet.hrValues[packet.hrValues.length - 1].bpm;
      } else {
        packet.hrValues = [];
        packet.bpm = null;
      }
      ...
      if (numValues <= 0) {
        ...
        const receivedTime = Date.now();
        let rate = 25;   // Assume default rate
        if (this.hrLastReceived && receivedTime > this.hrLastReceived) {
          rate = 1000 * numValues / (receivedTime - this.hrLastReceived);
        }
        if (rate < 1) rate = 1;
        if (rate > 100) rate = 100;
        this.hrLastReceived = receivedTime;

        for (let i = 0; i < numValues; i++) {
          const time = receivedTime - (1000 * (numValues + 1 - i) / rate);
          let j = 4 * i;
          ...
          values.push({ sampleIndex: this.hrSampleIndex++, time, bpm, als, hrs });
          lastBpm = bpm;
        }
        log('STREAMER: RECV-HR (' + bytes.length + 'b =' + numValues + ', bpm=' + lastBpm + ', rate~=' + rate + ')');
        //log('... ' + JSON.stringify(values));
        this.hrValuesBuffer.push(...values);
        this.hrLastReceived = Date.now();
      }
  ...
  packetReceived(packet) {
    //log('STREAMER: RECV-PACKET: ' + JSON.stringify(packet));
    this.bpm = packet.bpm;
    if (packet.bpm != null) this.lastBpm = packet.bpm;
    this.lastBattery = {
      raw: packet.battRaw,
    ...
    for (let i = 0; i < packet.samples.length; i++) {
      ...
      let estimatedTime = this.firstTime + reconstructedTime + (1000 * (-packet.samples.length + i + 1) / this.streamRate); // milliseconds, real-time estimate relative to first sample received (has long term drift)
      ...
      // Hackily squeeze hrValues into samples
      let hrIndex = -1;
      if (packet.hrValues != null && packet.hrValues.length > 0) {
        // Interpolate
        hrIndex = Math.floor(i * packet.hrValues.length / packet.samples.length);
        /*
        // Find closest time (only works if the time estimates are similar for two types of stream)
        for (let j = 0; j < packet.hrValues.length; j++) {
          if (packet.hrValues[j].time <= estimatedTime) {
            hrIndex = j;
          }
        }
        */
      }
      let hrSampleIndex = null;
      let hrTime = null;
      let hrBpm = null;
      let hrAls = null;
      let hrHrs = null;
      if (hrIndex >= 0 && hrIndex != lastHrIndex) {
        const hrValue = packet.hrValues[hrIndex];
        hrSampleIndex = hrValue.sampleIndex;
        hrTime = hrValue.time;
        hrBpm = hrValue.bpm > 0 ? hrValue.bpm : null;
        hrAls = hrValue.als > 0 ? hrValue.als : null;
        hrHrs = hrValue.hrs > 0 ? hrValue.hrs : null;
      }
      lastHrIndex = hrIndex;
