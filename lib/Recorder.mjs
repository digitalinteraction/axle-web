...
label.value = title
  ...
  newSamples(newSamples, streamer) {
    ...
    if (streamer.lastBpm !== null) {
      note = note + ' (' + streamer.lastBpm + ' bpm)';
    }
    ...
    if (this.state >= RecordingState.RECORDING) {
      ...
      for (let sample of newSamples) {
        ...
        const bpm = (streamer.lastBpm == null) ? '' : ('' + streamer.lastBpm);
        const line = '' + sample.sampleNumber + ',' + sample.packetSampleIndex + ',' + this.recordingName + ',' + streamer.device.deviceId() + ',' + sample.receivedTimestamp + ',' + sample.rawTimestamp + ',' + sample.sampleTime.toFixed(4) + ',' +  sample.estimatedTime.toFixed() + ',' + sample.accel[0].toFixed(4) + ',' + sample.accel[1].toFixed(4) + ',' + sample.accel[2].toFixed(4) + ',' + bpm + '\r\n';
        chunk += line;
      }
      //console.log(chunk);
