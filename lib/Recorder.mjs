...
label.value = title
  ...
  newSamples(newSamples, streamer) {
    if (streamer.lastBattery !== null) {
      ...
      this.graphs[streamer.id].setNote(streamer.lastBattery.percent + '%');
    }
    ...
    if (this.state >= RecordingState.RECORDING) {
      ...
      for (let sample of newSamples) {
        ...
        const line = '' + sample.sampleNumber + ',' + sample.packetSampleIndex + ',' + this.recordingName + ',' + streamer.device.deviceId() + ',' + sample.receivedTimestamp + ',' + sample.rawTimestamp + ',' + sample.sampleTime.toFixed(4) + ',' +  sample.estimatedTime.toFixed() + ',' + sample.accel[0].toFixed(4) + ',' + sample.accel[1].toFixed(4) + ',' + sample.accel[2].toFixed(4) + '\r\n';
        chunk += line;
      }
      //console.log(chunk);
