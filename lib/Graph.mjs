...
if (0)
      ...
      if (this.stats) {
        ...
        // Find range in each axis
        for (let i = this.samples.length - 1; i >= 0; i--) {
          timeAgo = now - this.samples[i].estimatedTime - timeOffset;
          if (timeAgo > timeWindow) break;
          sampleCount++;
          for (let chan = 0; chan < stats.length; chan++) {
            if (stats[chan].min === null || this.samples[i].accel[chan] < stats[chan].min) stats[chan].min = this.samples[i].accel[chan];
            if (stats[chan].max === null || this.samples[i].accel[chan] > stats[chan].max) stats[chan].max = this.samples[i].accel[chan];
          }
        }

        const lineHeight = 17;
        ctx.fillText(`#${sampleCount}/${(timeAgo / 1000).toFixed(1)}s`, 10, 40); 
        for (let chan = 0; chan < stats.length; chan++) {
          ctx.fillText(`${String.fromCharCode(chan < 3 ? 88 + chan : 90 - chan)}:(${stats[chan].min.toFixed(3)}-${stats[chan].max.toFixed(3)})=${(stats[chan].max - stats[chan].min).toFixed(3)}`, 10, 40 + lineHeight * (1 + chan)); 
        }
}
