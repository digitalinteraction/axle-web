...
rem start "http-server" cmd /c http-server .

::: Ngrok to make public
rem start "ngrok" ngrok http 8080

start "http-server-ngrok" wt new-tab --title "http-server-ngrok" -d . cmd /c "title http-server-ngrok && http-server ." ; split-pane --title "http-server-ngrok" ngrok http 8080
...
echo Manually refresh if changes are made (or run 'watch' for live reload).
